...
/**
 * Function: load_dirobj_img
 *
 * Returns:
 * @return diremission - a direct emission model
 */
dirobject *
load_dirobj_img(const object* actobject, fits_access* modim_access)
{
	int modimg;

	dirobject      *actdir=NULL;
	gsl_vector     *extention;
	dirim_emission *diremission=NULL;

	int nrel;
	int hdutype;
	int f_status=0;

	// get the index where to get the model
	modimg = actobject->beams[0].modimage;
	move_in_fits(modim_access, modimg);

	// allocate memory for the return structure
	actdir = (dirobject *) malloc(sizeof(dirobject));
	if (actdir == NULL)
		aXe_message (aXe_M_FATAL, __FILE__, __LINE__,
				"load_dirobj_img:" " Could not allocate"
				" memory for a dirobject object!");

	// allocate memory for the dirim structure
	diremission = (dirim_emission *) malloc(sizeof(dirim_emission));
	if (diremission == NULL)
		aXe_message (aXe_M_FATAL, __FILE__, __LINE__,
				"load_dirobj_img:" " Could not allocate"
				" memory for a dirim_emission object!");

	// load the image from the FITS file
	diremission->modimage = FITSHDU_to_gsl(modim_access->fits_file, modim_access->fits_ptr, 1);
	...
	// derive and store min/max in x/y for the corners
	actdir->ix_min = (int)floor(actobject->beams[0].refpoint.x - diremission->xmean + gsl_vector_get(extention, 0) + 0.5);
	actdir->ix_max = (int)floor(actobject->beams[0].refpoint.x + diremission->xmean + gsl_vector_get(extention, 1) + 0.5);
	actdir->iy_min = (int)floor(actobject->beams[0].refpoint.y - diremission->ymean + gsl_vector_get(extention, 2) + 0.5);
	actdir->iy_max = (int)floor(actobject->beams[0].refpoint.y + diremission->ymean + gsl_vector_get(extention, 3) + 0.5);
 ...
 * @param actobject - the object to be transformed
 * @param actdir    - the dirobject to store the flux values in
 * @param m_access  - the model acces structure
 * @param int_type  - the interpolation type to be used
 */
void
load_spectrum(const object *actobject, dirobject *actdir,
		fits_access *m_access, const int int_type)
{
  ...
  if (onebeam.flux != NULL && j < actobject->nbeams)
    {
      ...
      if (onebeam.modspec > 0 && m_access)
        {
          // get the model spectrum
          sed = load_model_sed(m_access, onebeam.modspec);
      ...
      else
        {
    	  ...
    	  // get the model spectrum from broad band colours
          sed = make_sed_from_beam(onebeam, int_type, actobject->ID);
 ...
 * Releases the memory allocated for a list of dirobjects. After releasing
 * memory, all elements are set to NULL.
 *
 * Note that here the memory for a model image is NOT released, in this
 * usage scenario only the references were stored. Also for high resolution
 * spectra only references are stored. For broad band spectra memory was
 * allocated and hence is free'd here!
 *
 * Parameters:
...
/**
 *
 * Function: free_dirobject
 * Releases the memory allocated for a direct object.
 *
 * Parameters:
 * @param actdir  - the list of dirobjects
 */
void
free_dirobject (dirobject *actdir){
	if (actdir->SED){
		free_enerdist(actdir->SED);
		actdir->SED = NULL;
	}
	if (actdir->dirim){
		free_dirim_emission(actdir->dirim);
		actdir->dirim = NULL;
	}
	free(actdir);
	actdir=NULL;
