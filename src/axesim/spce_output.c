		   ...
		   double rdnoise, const int bckmode)
{
  ...
  for (cur_p = ap_p; cur_p->p_x != -1; cur_p++)
	...
	continue;
      ...
      gsl_matrix_set(drzprep_stamps->error,i,j,(cur_p->error*cur_p->error*sqr_expt)/corr);


      // inverse variance weighting:
      // compute the error value
      //gsl_matrix_set(drzprep_stamps->error,i,j,(1.0/(cur_p->error*cur_p->error)));
