...
*/
gsl_matrix *
FITSimage_to_gslOld(const char *const fname, int hdunum, int fatal)
{
  fitsfile *input;
  int f_status = 0;
  float nulval = 0;
...
FITSimage_to_gsl(const char *const fname, int hdunum, int fatal)
{
  ...
  // open the fits file; complain if problems
  fits_open_file (&input, fname, READONLY, &f_status);
  if (f_status) {
	  if (fatal) {
		  fits_report_error (stderr, f_status);
  ...
  // move to the correct HDU; complain if problems
  fits_movabs_hdu (input, hdunum, &hdutype, &f_status);
  if (f_status) {
	  if (fatal) {
		  fits_report_error (stderr, f_status);
  ...
  // read the image data from the current HDU
  im = FITSHDU_to_gsl(fname, input, fatal);
...
/**
  ...
  @param fname File name of the FITS image
  @param fptr the number of the extension to read in (first primary = 1)
  @param fatal if non zero an error loading the data will trigger an exception
  @return a gsl_matrix containing the image data or NULL
*/
gsl_matrix *
FITSHDU_to_gsl(const char *const fname, fitsfile *fptr, int fatal)
{
  int f_status = 0;
  float nulval = 0;
  int anynul;
  ...
  // determine the image information;
  // complain if naxis is different from 1
  ffgipr (fptr, 2, &bitpix, &naxis, naxes, &f_status);
  if (naxis != 2) {
	  if (fatal) {
		  fits_report_error (stderr, f_status);
		  aXe_message (aXe_M_FATAL, __FILE__, __LINE__,
  ...
  // transport the entire image data to the storage
  ffgpxv (fptr, TFLOAT, zero, naxes[0] * naxes[1], &nulval, storage, &anynul, &f_status);
  if (f_status) {
	  if (fatal) {
		  fits_report_error (stderr, f_status);
		  aXe_message (aXe_M_FATAL, __FILE__, __LINE__,
				  "FITSimage_to_gsl: " "Could not read data from an extention "
				  " in file: %s", fname);
	  } else {
		  fits_report_error (stderr, f_status);
		  aXe_message (aXe_M_WARN4, __FILE__, __LINE__,
  ...
  // transfer the storage to a gsl-image
  im = storage_to_gsl(naxes, storage);

  // mop up and return the gsl-image
  free (storage);
  storage = NULL;
  return im;
}
...
/**
  ...
  @param naxes   axes of the image
  @param storage pointer to the data
*/
gsl_matrix *
storage_to_gsl(long *naxes, PIXEL_T *storage){
	int x, y;
	gsl_matrix *im;
	PIXEL_T *dp;

	// define the gsl matrix
	im = gsl_matrix_alloc (naxes[0], naxes[1]);
...
/**
  ...
  @param fname File name of the FITS image
  @param hdunum the number of the extension to read in (first primary = 1)
  @return the dimensation of the image in the specified extension
*/
px_point 
