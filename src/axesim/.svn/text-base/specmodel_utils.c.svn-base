...
/**
 * Function: has_aper_dirim
 * The function checks whether an object has an associated direct emission
 * structure or not. The corresponding boolean integer value is returned.
 *
 * Parameters:
 * @param m_access  - the fits access structure
 * @param actobject - the object structure
 *
 * Returns:
 * @return has_dirim - boolean whether direct emission exists (=1) or not (=0)
 */
int
has_aper_dirim(const fits_access *m_access, const object *actobject)
{
  int has_dirim=0;
...
/**
 * Function: access_fits_models
 *
 * Parameters:
 * @param  spectral_models_file - pathname to the spectral models file
 *
 * Returns:
 * @return model_map - the map of the models
 */
fits_access *
access_fits_models(const char fits_models_file[])
{
  int f_status=0;
  //int n_models=0;
  fits_access * m_access;

  // allocate space for the spectral models structure
  m_access = (fits_access *)malloc(sizeof(fits_access));

  // copy the file name to the struct
  strcpy(m_access->fits_file, fits_models_file);

  // open the fits file
  // report any error
  fits_open_file (&m_access->fits_ptr, fits_models_file, READONLY, &f_status);
  if (f_status)
    {
      ffrprt (stderr, f_status);
      aXe_message (aXe_M_FATAL, __FILE__, __LINE__,
		   "aXe_DISPIMAGE: " "Could not open file: %s",
		   fits_models_file);
    }

  // get the number of models
  m_access->n_modelHDU =  get_num_extensions(fits_models_file);
  m_access->act_hdu =  -1;
...
/**
 * Function: free_model_access
 * Releases the memory allocated in a model
 * access structure
 *
 * Parameters:
 * @param  m_access - the model access structure
 *
 * Returns:
 * @return -
 */
...
free_fits_access(fits_access *m_access)
{
	...
	// close the fits file
	fits_close_file (m_access->fits_ptr, &f_status);
	if (f_status)
	{
		ffrprt (stderr, f_status);
		aXe_message (aXe_M_FATAL, __FILE__, __LINE__,
				"aXe_DISPIMAGE: " "Could not close file: %s",
				m_access->fits_file);
	}
...
/**
 * Function: load_model_sed
 * Loads and returns the desired model spectrum from the spectral model table.
 * Translates from the object index [1, n_models] to the fits table
 * index [2, n_model+1] (taking into account the primary extension)
 *
 * Parameters:
 * @param  m_access   - the model access structure
 * @param  modspec    - the index of the desired SED
...
load_model_sed(fits_access *m_access, const int modspec)
{
	...
	move_in_fits(m_access, modspec);
	/*
	// make sure the SED does exist
	if (modspec < 0 || modspec >  m_access->n_modelHDU)
		aXe_message (aXe_M_FATAL, __FILE__, __LINE__,
				"aXe_DISPIMAGE: " "Model number %i does NOT exist!",
				modspec);

	// if accessing the first time
	// we go to an absolute position
	if (m_access->act_hdu <0){
		// move to the absolute HDU positions
		fits_movabs_hdu(m_access->fits_ptr, modspec+1, &hdutype, &f_status);
		if (f_status) {
			ffrprt (stderr, f_status);
			aXe_message (aXe_M_FATAL, __FILE__, __LINE__,
					"aXe_DISPIMAGE: " "Could not move to HDU %d in file: %s!",modspec+1, m_access->fits_file);
		    }
		//fprintf (stdout,"In file: %s moving to HDU: %d.\n", m_access->fits_file, m_access->act_hdu);
	}
	// move relative to the current position
	else{
		// compute the relative move and jump there
		nrel=modspec+1-m_access->act_hdu;
		fits_movrel_hdu(m_access->fits_ptr, nrel,  &hdutype, &f_status);
		if (f_status) {
			ffrprt (stderr, f_status);
			aXe_message (aXe_M_FATAL, __FILE__, __LINE__,
					"aXe_DISPIMAGE: " "Could not move %d HDU's in file: %s!", nrel, m_access->fits_file);
		    }
		//fprintf (stdout,"In file: %s moved relative: %d.\n",m_access->fits_file, nrel);
	}
	// update the current HDU number
	m_access->act_hdu=modspec+1;
	*/

    // load the energy distribution
	sed = load_SED_from_fitsext(m_access->fits_file, m_access->fits_ptr);
 ...
 * Parameters:
 * @param  m_access   - the model access structure
 * @param  targetHDU  - index of the target HDU
 *
 * Returns:
 * @return -
 */
...
move_in_fits(fits_access *m_access, const int targetHDU)
{
	...
	// make sure the SED does exist
	if (targetHDU < 0 || targetHDU >  m_access->n_modelHDU)
		aXe_message (aXe_M_FATAL, __FILE__, __LINE__,
				"aXe_DISPIMAGE: " "Can not move to HDU %i!",
				targetHDU);
	...
	if (m_access->act_hdu <0){
		// move to the absolute HDU positions
		fits_movabs_hdu(m_access->fits_ptr, targetHDU+1, &hdutype, &f_status);
		if (f_status) {
			ffrprt (stderr, f_status);
			aXe_message (aXe_M_FATAL, __FILE__, __LINE__,
					"aXe_DISPIMAGE: " "Could not move to HDU %d in file: %s!",targetHDU+1, m_access->fits_file);
		    }
		//fprintf (stdout,"In file: %s moving to HDU: %d.\n", m_access->fits_file, m_access->act_hdu);
	}
	// move relative to the current position
		...
		// compute the relative move and jump there
		nrel=targetHDU+1-m_access->act_hdu;
		fits_movrel_hdu(m_access->fits_ptr, nrel,  &hdutype, &f_status);
		if (f_status) {
			ffrprt (stderr, f_status);
			aXe_message (aXe_M_FATAL, __FILE__, __LINE__,
