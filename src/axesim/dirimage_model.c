...
int 
compute_dirimage_modelOld(char dirim_file[], char conf_file[], char tpass_file[],
		...
		observation *obs, char map_file[])
{
	...
	get_extension_numbers(dirim_file, conf,conf->optkey1,conf->optval1);

	// get the  matrix with the drizzle coefficients
	drzcoeffs = get_crossdisp_matrix(dirim_file, conf->science_numext);
	...
	for (i = 0; i < nobjects; i++){
		...
		if (oblist[i]->nbeams > 0){
			...
			if (has_aper_dirim(modim_access, oblist[i]))
				...
				actdir = fill_dirobject(oblist[i], npixels, drzcoeffs, model_scale, max_offs);

			// load the spectral values into the dirobject
			load_spectrum(oblist[i], actdir, spectrum_access, 1);

			// go over each beam defined in the configuration file
			for (beamID=0; beamID < conf->nbeams; beamID++) {

				// set the offset values to 0.0
				...
				m_point = get_dirobject_meanpos(actdir);

				// evaluate the coefficients for the 2D variable offsets
				// at the mean position
				actdir->xy_off[beamID].x = eval_trace_off_at_pos (x_coeffs, m_point, beamID);
				actdir->xy_off[beamID].y = eval_trace_off_at_pos (y_coeffs, m_point, beamID);
			...
			// add the current direct object to the image
			make_dirimage_object(oblist[i], actdir, tpass, dirimage_matrix);
		}
		// free the direct object
...
void
make_dirimage_object(object *actobject, dirobject *actdir, interpolator *tpass, gsl_matrix *dirimage_matrix){
	...
	actbeam = actobject->beams[0];

	// report onto the screen
	fprintf(stdout, "aXe_DIRIMAGE: modelling object %i ...", actobject->ID);
  ...
  while (oblist[ii] != NULL)
    {
      ...
      // report onto the screen
      fprintf(stdout, "NNNNEW aXe_DIRIMAGE: modelling object %i ...", oblist[ii]->ID);
