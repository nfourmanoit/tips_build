...
class DummyImages(object):
    ...
    def _get_image_data(self, conf):
        ...
        # check whether the mode is NISP/GBLUE
        elif self.conf['INSTRUMENT'] == 'NISP' and self.conf['CAMERA'] == 'GBLUE':
            image_data = WCSdata.get_NISP_GBLUE_WCS(conf)

        # check whether the mode is NISP/GRED
        elif self.conf['INSTRUMENT'] == 'NISP' and self.conf['CAMERA'] == 'GRED':
            image_data = WCSdata.get_NISP_GRED_WCS(conf)
...
#################################
    def makeOneImage(self, imgname, nx, ny, metadata, drzmeta=None):
        ...
        # go the the header and put
        # the exposure time
        hdr0 = mex_hdu[0].header
        hdr0.update('EXPTIME', 1.0, 'dummy exposure time')

        if drzmeta != None:
            # update the header
            for item in drzmeta:
                hdr0.update(item[0], item[1], item[2])

        # create a hdu image with the right dimension
        img = numpy.zeros((ny,nx),dtype=numpy.float32)
        hdu = pyfits.ImageHDU(img)
        mex_hdu.append(hdu)
        ...
        # open the dummy image, go to the header
        hdr1 = mex_hdu[1].header

        # update the header
        for item in metadata:
            hdr1.update(item[0], item[1], item[2])

        hdr1.update('EXTNAME', 'SCI', 'name of this extension')
