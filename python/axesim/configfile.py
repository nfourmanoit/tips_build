...
class ConfigList(object):
    ...
    def writeto(self, filename, b=None):
        """
    ...
    def _find_gkeys(self, keylist):
        ...
        gkeywords = ['INSTRUMENT', 'CAMERA', 'TELAREA',
                     ...
                     'POBJSIZE', 'SMFACTOR', 'PSFSIG1', 'PSFSIG2', 'PSFC',
                     'NPIXX', 'NPIXY', 'DC', 'QE',
                     'CRPIX1', 'CRPIX2', 'CRVAL1', 'CRVAL2', 'CD1_1',
                     'CD1_2', 'CD2_1', 'CD2_2', 'ORIENTAT', 'EXPNAME',
                     'COSMAP', 'REJECT', 'NBIT',
                     'RDMODE', 'NGRP', 'NFRM', 'DTGRP', 'DTFRM']
...
class ConfigFile(ConfigList):
    ...
    def axesim_prep(self, b=None):
        ...
        self.add_gkey('EXPTIME', 'EXPTIME')
        # write the file back
        self.writeto(new_name, b)
...
class ConfigBeam(object):
    ...
    def _find_beamkeys(self, ident, keylist):
        ...
        okeys = ['PSF_OFFSET_', 'PSFSIG1', 'PSFSIG2', 'PSFC']
        ...
        for key in keylist:
            ...
            if key.keyword in id_keys:
                ...
                # enhance the number of keywords found
                nfound += 1
        ...
        for key in fkeys:
            ...
            for bkey in self.beamkeys:
                ...
                if bkey.keyword == full_keyword and bkey.keyvalue.upper() != 'NONE':
                    if not os.path.isfile(putCONF(bkey.keyvalue)):
                        error_message = 'The file does not exist: ' + str(putCONF(bkey.keyvalue))
    ...
    def toBeamA(self):
        ...
        rstring += self.trace.toBeamA()

        # add the string for the dispersion
        # solution
        rstring += self.disp.toBeamA()
...
class TwoDimPolyN(object):
    ...
    def __getitem__(self, index):
        ...
        if index > len(self.twodkeys)-1:
            # raise an exception
            raise 'Index: ' + str(index) + ' does not exist!'
    ...
    def __setitem__(self, index, obj):
        ...
        # check whether the input type is correct
            ...
            # raise an exception
            raise 'Object: ' + str(obj) + ' has wrong type: ' +str(type(obj))+'!'
    ...
    def toBeamA(self):
        ...
        description = 'Trace description for Beam A'

        rstring = self.norder.toBeamA()
        for key in self.twodkeys:
            rstring += key.toBeamA_Poly()
        return rstring
...
class ConfigTrace(TwoDimPolyN):
    ...
    def __init__(self, ident=None, keylist=None):
        ...
        except CKeyNotFound, e:
            raise TraceNotFound(ident, e.keyword)
        except CKeyLengthWrong, e:
            ...
            for line in fopen:
                ...
                if start:
                    ...
                    if len(str_line) > 0 and str_line[0] == '#':
                        ...
                        self.header.append(string.strip(line)+'\n')
                    else:
...
class ConfKey(object):
    ...
    def toBeamA(self):
        rstring = self.keyword[:-1]+'A ' + str(self.keyvalue)
        rstring += '\n'
        return rstring

    def toBeamA_Poly(self):
        rstring = self.keyword[:5]+'A'+self.keyword[6:]+' '+str(self.keyvalue)
        rstring += '\n'
        return rstring
...
class ConfListKey(ConfKey):
    ...
    def __init__(self, keyword, keyvalue, comment=None):
        ...
        vlist = string.split(self.keyvalue)
        for value in vlist:
