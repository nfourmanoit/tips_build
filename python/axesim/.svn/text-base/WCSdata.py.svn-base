"""
$Revision: 1.2 $ $Date: 2009/09/14 09:48:02 $
...
def get_NISP_GBLUE_WCS(conf=None):
    """
    Defines parameters for the NISP slitless mode with blue grism
    ...
    """
    wcs_keys = {}
    #WCS from NISP grism image TBD
    #/ World Coordinate System and Related Parameters
    
    # if wcs are defined in the configuration file, use it
    # else define dummy wcs
    if conf != None:
      if conf.get_gvalue('CRPIX1') != None and conf.get_gvalue('CRPIX2') != None and \
         conf.get_gvalue('CRVAL1') != None and conf.get_gvalue('CRVAL2') != None and \
         conf.get_gvalue('CD1_1') != None and conf.get_gvalue('CD1_2') != None and \
         conf.get_gvalue('CD2_1') != None and conf.get_gvalue('CD2_2') != None and \
         conf.get_gvalue('ORIENTAT') != None and conf.get_gvalue('EXPNAME') != None :
        print 'SIMDISPIM: WCS set from configuration files'
        
        wcs_keys['grism'] = [
          ['WCSAXES',                   2, 'number of World Coordinate System axes'],
          ['CRPIX1',     float(conf.get_gvalue('CRPIX1')), 'x-coordinate of reference pixel'],
          ['CRPIX2',     float(conf.get_gvalue('CRPIX2')), 'y-coordinate of reference pixel'],
          ['CRVAL1',     float(conf.get_gvalue('CRVAL1')), 'first axis value at reference pixel'],
          ['CRVAL2',     float(conf.get_gvalue('CRVAL2')), 'second axis value at reference pixel'],
          ['CTYPE1', 'RA---TAN'                          , 'the coordinate type for the first axis'],
          ['CTYPE2', 'DEC--TAN'                          , 'the coordinate type for the second axis'],
          ['CD1_1',       float(conf.get_gvalue('CD1_1')), 'partial of first axis coordinate w.r.t. x'],
          ['CD1_2',       float(conf.get_gvalue('CD1_2')), 'partial of first axis coordinate w.r.t. y'],
          ['CD2_1',       float(conf.get_gvalue('CD2_1')), 'partial of second axis coordinate w.r.t. x'],
          ['CD2_2',       float(conf.get_gvalue('CD2_2')), 'partial of second axis coordinate w.r.t. y'],
          ['LTV1',                                    0.0, 'offset in X to subsection start'],
          ['LTV2',                                    0.0, 'offset in Y to subsection start'],
          ['LTM1_1',                                  1.0, 'reciprocal of sampling rate in X'],
          ['LTM2_2',                                  1.0, 'reciprocal of sampling rate in Y'],
          ['ORIENTAT', float(conf.get_gvalue('ORIENTAT')), 'position angle of image y axis (deg. e of n)'],
          ['EXPNAME',          conf.get_gvalue('EXPNAME'), 'calibration exposure identifier'],
          ]
        
      else:
        print 'SIMDISPIM: WCS set (dummy)'
        
        wcs_keys['grism'] = [
          ['WCSAXES',                   2, 'number of World Coordinate System axes'],
          ['CRPIX1',                  0.0, 'x-coordinate of reference pixel'],
          ['CRPIX2',                  0.0, 'y-coordinate of reference pixel'],
          ['CRVAL1',   1.000000000000E+02, 'first axis value at reference pixel'],
          ['CRVAL2',   2.000000000000E+00, 'second axis value at reference pixel'],
          ['CTYPE1', 'RA---TAN'          , 'the coordinate type for the first axis'],
          ['CTYPE2', 'DEC--TAN'          , 'the coordinate type for the second axis'],
          ['CD1_1',           8.33333E-05, 'partial of first axis coordinate w.r.t. x'],
          ['CD1_2',           0.00000E-00, 'partial of first axis coordinate w.r.t. y'],
          ['CD2_1',           0.00000E-00, 'partial of second axis coordinate w.r.t. x'],
          ['CD2_2',           8.33333E-05, 'partial of second axis coordinate w.r.t. y'],
          ['LTV1',                    0.0, 'offset in X to subsection start'],
          ['LTV2',                    0.0, 'offset in Y to subsection start'],
          ['LTM1_1',                  1.0, 'reciprocal of sampling rate in X'],
          ['LTM2_2',                  1.0, 'reciprocal of sampling rate in Y'],
    ...
    # Dummy drizzle parameters (need for some aXe task)
    wcs_keys['drizzle'] = [
        ['DRZCNUM',                   15, 'Number of coefficients per coordinate'],
        ['DRZSCALE',                 0.3, 'Scale for drizzling'],
        ['DRZ2X01',           0.14763942, 'Drizzle coefficient 01 in X'],
        ['DRZ2X02',            1.0560001, 'Drizzle coefficient 02 in X'],
        ['DRZ2X03',       -0.00016301818, 'Drizzle coefficient 03 in X'],
        ['DRZ2X04',       -1.9466206E-07, 'Drizzle coefficient 04 in X'],
    ...
    wcs_keys['dimension'] = [int(conf.get_gvalue('NPIXX')), int(conf.get_gvalue('NPIXY'))]

    return wcs_keys

def get_NISP_GRED_WCS(conf):
    """
    Defines parameters for the NISP slitless mode with red grism
